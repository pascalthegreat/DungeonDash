...
running_man = [pygame.transform.scale(pygame.image.load(os.path.join('running_man', x)), (PLAYER_WIDTH, PLAYER_HEIGHT))
...
def draw_window(players, camera_speed, camera_position, frame_num, monster, obstacles):
    WIN.fill(BLACK)
    ...
    for player in players:
        ...
        rotimage = pygame.transform.rotate(running_man[int(player.walk_count // 3)],
                                           player.angle + 180)
...
def run_sim(seed_num, speed):
    camera_position = 0

    p1 = Player(700, 500, P1_KEYS)
    p2 = Player(100, 500, P2_KEYS)
    ...
    run = True
    frame_num = 0
    obstacles = generate_obstacles(seed_num, 50)
    ...
    prev_y2_frames = 0
    # gaps = get_gaps(obstacles)
    # while run and (p2.y > -10000 or p1.y > -10000) and frame_num < 4000\
    #         and prev_y1_frames < 500 and prev_y2_frames < 500:
    while run and p2.y > -10000 and frame_num < 4000 and prev_y2_frames < 500:
        frame_num += 1
        ...
        # handle_player_movement(computer_keys1, p1, obstacles, speed)
        handle_player_movement(computer_keys2, p2, obstacles, speed)
        ...
        if prev_y1 - 30 < p1.y < prev_y1 + 30:
            prev_y1_frames += 1
        else:
            prev_y1 = p1.y
            prev_y1_frames = 0
        if prev_y2 - 30 < p2.y < prev_y2 + 30:
            prev_y2_frames += 1
        else:
            prev_y2 = p2.y
            prev_y2_frames = 0
        # draw_window(players, camera_speed, camera_position, frame_num, monster, obstacles)
        # print(players[0].y)
        # print(players[0].ver_vel, players[1].ver_vel, players[0].screen_y, players[1].screen_y,
        #       players[0].y, players[1].y, camera_speed)
    print(prev_y1_frames, prev_y2_frames)
    return frame_num if frame_num < 4000 and prev_y2_frames < 500 else None


def main():
    json_path = 'sims_results.json'
    if os.path.exists(json_path):
        with open(json_path, encoding='utf8') as infile:
            sims_results = json.load(infile)
    else:
        sims_results = {}
        with open(json_path, 'w', encoding='utf8') as outfile:
            json.dump(sims_results, outfile)
    if sims_results:
        print(max([int(x) for x in list(sims_results)]))
    seed_num = 0 if not sims_results else max([int(x) for x in list(sims_results)]) + 1
    while True:
        speed = 8
        try:
            frame_num = run_sim(seed_num, speed)
        except UnboundLocalError:
            frame_num = None
        print(seed_num, speed, frame_num)
        sims_results[seed_num] = [{'speed': speed, 'frames': frame_num}]
        if frame_num:
            for speed in [9, 10]:
                try:
                    res = run_sim(seed_num, speed)
                except UnboundLocalError:
                    res = None
                print(seed_num, speed, res)
                sims_results[seed_num].append({'speed': speed, 'frames': res})
        with open(json_path, 'w', encoding='utf8') as outfile:
            json.dump(sims_results, outfile)
        seed_num += 1
